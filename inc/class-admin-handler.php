...
class AdminHandler {
	...
	public function register_hooks(): void {
		...
		add_action( 'admin_notices', array( $this, 'handle_activation_redirect_fallback' ) );

		// Post/Page columns
		add_filter( 'manage_posts_columns', array( $this, 'add_emissions_columns' ) );
		add_filter( 'manage_pages_columns', array( $this, 'add_emissions_columns' ) );
		add_action( 'manage_posts_custom_column', array( $this, 'render_emissions_column_content' ), 10, 2 );
		add_action( 'manage_pages_custom_column', array( $this, 'render_emissions_column_content' ), 10, 2 );

		// Plugin action links
		add_filter( 'plugin_action_links_' . plugin_basename( CARBONFOOTER_PLUGIN_FILE ), array( $this, 'add_plugin_action_links' ) );
	...
	private function get_menu_icon_svg(): string {
		...
		// Embed SVG icon as base64 data URL for admin menu icon.
	...
	private function localize_admin_script( string $hook_suffix ): void {
		$localization_data = array(
			...
			'initialData'         => array(
				...
				'resource_stats'   => array(), // Will be loaded via AJAX
			),
			'i18n'                => array(
	...
	private function register_appearance_settings(): void {
		$appearance_settings = array(
			Constants::OPTION_WIDGET_BACKGROUND_COLOR => array(
				'type'              => 'string',
				'default'           => Constants::DEFAULT_BACKGROUND_COLOR,
				'sanitize_callback' => 'sanitize_hex_color',
			),
			Constants::OPTION_WIDGET_TEXT_COLOR       => array(
				'type'              => 'string',
				'default'           => Constants::DEFAULT_TEXT_COLOR,
				'sanitize_callback' => 'sanitize_hex_color',
			),
			Constants::OPTION_DISPLAY_SETTING         => array(
				'type'              => 'string',
				'default'           => Constants::DEFAULT_DISPLAY_SETTING,
				'sanitize_callback' => 'sanitize_text_field',
			),
			Constants::OPTION_WIDGET_STYLE            => array(
				'type'              => 'string',
	...
	private function register_privacy_settings(): void {
		$privacy_settings = array(
			Constants::OPTION_DATA_COLLECTION_ENABLED => array(
				'type'              => 'boolean',
				'default'           => Constants::DEFAULT_DATA_COLLECTION_ENABLED,
				'sanitize_callback' => 'rest_sanitize_boolean',
			),
			Constants::OPTION_SHOW_ATTRIBUTION        => array(
				'type'              => 'boolean',
	...
	public function handle_activation_redirect_fallback(): void {
		...
		wp_enqueue_script( 'carbonfooter-admin-redirect', '', array(), CARBONFOOTER_VERSION, true );
